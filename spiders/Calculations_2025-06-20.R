...
model_viz_supp <- function(df0){
    ...
    df1 %>% 
        ggplot(aes(x = km, y = predicted,  color = model)) + #
...
# Rarefication ------------------------------------------------------------
library(parallel)
cl <- makeCluster(min(detectCores()-1, 8))
rar <- long %>% 
    ...
    map(~list(data = .x, nnn = if(export){999}else{9})) %>% 
